...
def call_openai(
    ...
    # 1. Tạo client OpenAI
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    # 2. Đo thời gian bắt đầu
    start_time = time.time()
    # 3. Gọi Chat Completions API
    ...
    # 4. Đo thời gian kết thúc
    latency = time.time() - start_time
    if latency <= 0:
        latency = 0.001
    # 5. Trích xuất nội dung phản hồi
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        try:
            ...
            assistant_reply = "(mocked response in test)"
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        gpt4o_lat = r.get("gpt4o_latency", 0)
        mini_lat = r.get("mini_latency", 0)
        
        line = f"{prompt:<42} | {gpt4o_resp:<42} | {mini_resp:<42} | {gpt4o_lat:>13.3f}s | {mini_lat:>11.3f}s"
        lines.append(line)
